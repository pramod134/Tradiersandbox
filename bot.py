...
EXTENDED_STOCK_ENABLED = str(EXTENDED_STOCK_ENABLE).lower() in ("1","true","yes")
...
def append_partial(partial: Dict[str,Any]):
    # Partials: partial_id | trade_id | type | timestamp | qty | fill_price | underlying_price | target_label | reason | commission_fee | realized_pnl_$ | notes
    row = [
...
def append_active_trade(trade_id: str, parsed: Dict[str,Any], qty: int, status: str="waiting", occ_symbol: str=None, strike: float=None, expiry: str=None):
    # ActiveTrades: trade_id | ticker | side | qty | entry_tf | entry_cond | entry_level | stop_tf | stop_cond | stop_level | tps_json | notes | started_at | occ_symbol | strike | expiry
...
# === bot.py (Part 2/5) ===
# OpenAI, tools/orchestrator, sanitization
...
def sanitize_parsed(p: Dict[str,Any]) -> Tuple[Optional[Dict[str,Any]], List[str]]:
    ...
    return p, warnings
# === bot.py (Part 3/5) ===
# Tradier helpers: quotes, chains, orders, positions, account, closures
...
def sandbox_list_positions_detailed():
    ...
    for p in pos:
        ...
        if p.get("price") not in (None, "", "null"):
            try:
                avg = float(p.get("price"))
            except:
                avg = None
        if avg is None and p.get("cost_basis") not in (None, "", "null"):
            try:
                cb = float(p.get("cost_basis"))
                avg = cb / qty if qty else None
            except:
                pass
...
class PositionStore:
    ...
    def add(self, tid:str, rec:Dict[str,Any]):
        self.state[tid]=rec
    def get(self, tid:str):
        return self.state.get(tid)
    def all_active(self):
        return {k:v for k,v in self.state.items() if v.get("status") in ("active","tp_partial")}
    def set(self, tid, key, val):
        if tid in self.state:
            self.state[tid][key]=val

positions = PositionStore()

# === bot.py (Part 4/5) ===
# Discord bot, parsers, watchers, and handlers
...
def try_immediate_stock_intent(text: str):
    patterns = [
        r"\b(buy|sell)\s+(?:(\d+)\s+)?([A-Za-z]{1,5})\s+shares\b",
        r"\b(buy|sell)\s+([A-Za-z]{1,5})\s+(\d+)\s+shares\b",
        r"\b(buy|sell)\s+(?:(\d+)\s+)?([A-Za-z]{1,5})\b(?!\s+calls?\b|\s+puts?\b)"
    ]
    for pat in patterns:
        ...
        if m:
            ...
            qty = None; ticker = None
            for g in groups:
                if g.isdigit():
                    qty = int(g)
                elif re.match(r"^[A-Za-z]{1,5}$", g):
                    ticker = g.upper()
            if not ticker:
                continue
            if qty is None:
...
def try_close_stock_intent(text: str):
    t = text.strip().lower()
    m = re.search(r"\b(sell|close)\b\s+(all|half|\d+%|\d+)?\s*([A-Za-z]{1,5})(?:\s+shares?)?\b", t, re.I)
    if not m:
        return None
    qtyword = m.group(2) or "all"
    ...
    if qtyword == "all":
        ...
        try:
            ...
            pct = 100
    return {"ticker": ticker, "pct": pct, "qty_abs": qty_abs}
...
def try_immediate_option_intent(text: str):
    ...
    if re.search(r"\bout of the money\b|\botm\b", text, re.I):
        delta = 0.30
    target_hint = "next_week" if re.search(r"\bnext\s+week\b", text, re.I) else None
    if target_hint:
        dte = 7
    return {
        "ticker": ticker,
...
def try_move_sl_be_intent(text: str):
    ...
    if not (m_tkr and m_sp and m_exp):
        return None
    ticker = m_tkr.group(1).upper()
    strike = float(m_sp.group(1))
    tchar = m_sp.group(2).lower()
...
def find_active_trades_by_contract(ticker: str, opt_type: str, strike: float, expiry: str):
    ...
    for a in acts:
        if a.get("ticker") != ticker.upper():
            continue
        occ = (a.get("occ_symbol") or "").upper()
        if occ:
            if (opt_type=="call" and not occ.endswith("C")) or (opt_type=="put" and not occ.endswith("P")):
                continue
...
async def watch_and_execute(trade_id: str, parsed: Dict[str,Any], channel: Optional[discord.TextChannel]):
    ...
    if not ticker:
        if channel:
            await channel.send("⚠️ Aborting: empty ticker.")
    ...
    while True:
        try:
            ...
            if channel:
                await channel.send(f"⚠️ Quote error for {ticker}: {e}")
    ...
    while True:
        ...
        for tp in list(tps):
            ...
            if touch and remaining>0:
                if "sell_qty" in tp:
                    slice_qty = min(int(tp["sell_qty"]), remaining)
                elif "sell_pct" in tp:
                    slice_qty = max(1, math.floor(remaining * (int(tp["sell_pct"])/100)))
                else:
                    slice_qty = 1
...
# === bot.py (Part 5/5) ===
# Close helpers by UL, message listener, main()
...
async def on_message(message: discord.Message):
    ...
    # YES/NO/OK should go to the orchestrator (avoid JSON parse errors on "yes")
    
    simple_ack = content.strip().lower()
    if simple_ack in {"yes","y","ok","sure","no","n","cancel","confirm"}:
        try:
            if simple_ack == "confirm":
                ...
                if not pend:
                    return await message.channel.send("ℹ️ No pending action to confirm.")
                # Optional: expire confirmations older than 5 minutes
                if time.time() - pend.get("ts", 0) > 300:
                    del PENDING_ACTIONS[ch]
                    return await message.channel.send("⌛ The pending action expired. Please re-issue the request.")
                ...
                if "occ_symbols" in pend:
                    ...
                    # Future: handle equity/other pending actions if you store them similarly
                    del PENDING_ACTIONS[ch]
                    return await message.channel.send("✅ Confirmed. (No actionable pending data found.)")
            elif simple_ack == "cancel":
                ch = str(message.channel.id)
                if PENDING_ACTIONS.pop(ch, None):
                    return await message.channel.send("🛑 Pending action canceled.")
                # fall through to orchestrator if nothing pending
            # route other acks (yes/no/ok/etc.) to the orchestrator as before
    ...
    # Manager router for info/trade requests
    lowered = content.lower()

# If we're mid-dialog (assistant asked a question last turn), route any reply to the orchestrator
try:
    ch = str(message.channel.id)
    ...
    if msl:
        ...
        if not matches:
            return await message.channel.send("ℹ️ No matching active option trade found for that contract.")
        count=0; notes=[]
        for a in matches:
            rownum, header = gs_find_row(TRADES_TAB, "trade_id", a["trade_id"])
            if rownum:
                update_trade_history(a["trade_id"], {"notes":"SL→BE"})
                count+=1; notes.append(f"• `{a['trade_id']}` → SL→BE noted")
        if count==0:
            return await message.channel.send("⚠️ Could not update trade to BE (trade not found in sheet).")
    ...
    if csi:
        ...
        if res["closed"] <= 0:
            return await message.channel.send(f"ℹ️ No {tkr} shares to sell (position not found or zero qty).")
    ...
    if isty:
        ...
        try:
            ...
            return await message.channel.send(f"⚠️ Equity order error: {e}")
        finally:
    ...
    # IMMEDIATE OPTION (defaults: 2w DTE, Δ0.5, market; “next week” honored; ITM/OTM → delta map)
    ...
    if iopt:
        ...
        try:
            ...
            return await message.channel.send(f"⚠️ Option order error: {e}")
    ...
    # Only allow the LLM signal parser on messages that look like trading instructions
    looks_like_trade = bool(re.search(
        r"\b(buy|sell|call|calls|put|puts|option|options|shares?|tp|take[- ]?profit|stop[- ]?loss|sl|entry|breakout|breakdown|close)\b",
        content, re.I
    ))
    if not looks_like_trade:
        return

    # Rule-based signals (LLM parser → watcher)
    try:
